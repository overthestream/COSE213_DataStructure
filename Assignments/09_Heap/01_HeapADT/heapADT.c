...
void reheapUp(HEAP* heap, int pos)
{
	...
	parentIdx = (idx-1)/2;
	while(idx>0 && curData> heap->heapAry[parentIdx]){
		heap->heapAry[idx] = heap->heapAry[parentIdx];
		idx = parentIdx;
		parentIdx = (idx-1)/2;
	}
	heap->heapAry[idx]=curData;
	*/
...
void reheapDown(HEAP* heap, int pos)
{
	...
	Element curData=0;
	if(idx<0 || idx>= heap->size) return;
	curData = heap->heapAry[idx];
	while(idx*2 +1 < heap->size){
		int maxChild  = idx*2 +1;
		if(maxChild+1<heap->size && heap->heapAry[maxChild+1]>heapAry[maxChild])
			maxChild++;

		if(heap->heapAry[maxChild] > curData){
			heap->heapAry[idx] = heap->heapAry[maxChild];
			idx = maxChild;
		}
		else break;
	}
